...
from selita.models import Users, Client, Product, Account, Transaction, Sales, Payment, Supplier, Inventory
...
class AccountingConsistencyTest(APITestCase):
    ...
    def test_delete_sale_reverses_accounting(self):
        ...
        response = self.client.delete(f'/selita/delete-sale/{sale.id}')
    ...
    def test_update_sale_simplified_currency_logic(self):
        ...
        response = self.client.put(f'/selita/update-sale/{sale.id}', update_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.put(f'/selita/update-sale/{sale.id}', error_data)
