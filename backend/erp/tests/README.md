...
This directory contains all tests for the ERP backend application.
...
```bash
python manage.py test erp.tests.unit.test_models
```

### Run specific test class
```bash
python manage.py test erp.tests.unit.test_models.ClientModelTests
```

### Run specific test method
```bash
python manage.py test erp.tests.unit.test_models.ClientModelTests.test_client_creation
```
...
pytest erp/tests/unit/test_models.py
...
class MyModelTests(ErpTestCase):
...
class MyAPITests(APITestCase, ErpTestCase):
    def setUp(self):
        super().setUpTestData()
