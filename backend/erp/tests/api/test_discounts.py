...
class DiscountCreateTests(ErpTestCase):
    ...
    def test_create_sale_no_discount(self):
        ...
        assert resp.data["items"][0]["discount_amount"] == 0
    ...
    def test_create_sale_percent_discount(self):
        ...
        assert resp.status_code == status.HTTP_201_CREATED
        assert resp.data["items"][0]["discount_amount"] == 15.0  # 10% of 150
    ...
    def test_create_sale_fixed_discount(self):
        ...
        assert resp.status_code == status.HTTP_201_CREATED
        assert resp.data["items"][0]["discount_amount"] == 25.0
    ...
    def test_percent_discount_with_tax(self):
        ...
        assert resp.data["items"][0]["discount_amount"] == 30.0
        assert resp.data["items"][0]["tax_amount"] == 24.0
    ...
    def test_fixed_discount_with_tax(self):
        ...
        assert resp.data["items"][0]["discount_amount"] == 50.0
        assert resp.data["items"][0]["tax_amount"] == 20.0
...
class DiscountDetailsTests(ErpTestCase):
    ...
    def test_details_include_discount_fields(self):
        ...
        details = self.api.get(f"/erp/sale-details/{tx_id}")
        assert details.status_code == status.HTTP_200_OK
        item = details.data["items"][0]
        assert item["discount_type"] == "PERCENT"
        assert float(item["discount_value"]) == 10.0
        assert float(item["discount_amount"]) == 10.0  # 10% of 100
...
class DiscountUpdateTests(ErpTestCase):
    ...
    def _create_sale(self, **extra):
        ...
        return resp.data["transaction_id"], resp.data["items"][0]["sale_id"]

    def test_add_discount_to_existing_sale(self):
        tx_id, sale_id = self._create_sale()
        resp = self.api.put(
            f"/erp/update-sale/{tx_id}",
            {
                "client_id": self.client_obj.id,
                "currency": "EUR",
                "items": [{
    ...
    def test_remove_discount_from_sale(self):
        tx_id, sale_id = self._create_sale(discount_type="PERCENT", discount_value="10")
        resp = self.api.put(
            f"/erp/update-sale/{tx_id}",
            {
                "client_id": self.client_obj.id,
